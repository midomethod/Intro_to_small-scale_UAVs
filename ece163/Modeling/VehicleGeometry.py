...
class VehicleGeometry():
	...
	def getNewPoints(self, x, y, z, yaw, pitch, roll):
		...
		oldPoints = self.vertices	# nx3 matrix of all the vertices
		Ri2b = Rotations.euler2DCM(yaw, pitch, roll) # Rotaion from inertial to body frame
		PointsAfterRotation = MatrixMath.matrixMultiply(oldPoints, Ri2b) # Apply the rotation from b to i frame
		Transpose = MatrixMath.matrixTranspose(PointsAfterRotation)	# Transpose the matrix so it's easier to add x,y,z
		
		# Split the list into the x,y,z coordinates
		T_x = Transpose[0]
		T_y = Transpose[1] 
		T_z = Transpose[2]  

		# Use list comprehension to perform all the offsets
		T_x = [elem+x for elem in T_x]
		T_y = [elem+y for elem in T_y]
		T_z = [elem+z for elem in T_z]

		PointsAfterDisplacementT = [T_x,T_y,T_z]	# Put the split coordinates into a list to put it back together
		PointsAfterDisplacement = MatrixMath.matrixTranspose(PointsAfterDisplacementT) # Transpose it back
		newPoints = Rotations.ned2enu(PointsAfterDisplacement) # Convert coordinate to ENU frame
